...
def handleWorker(worker_id,port,gradients_q,worker_id_q,global_var_vals,global_flags):
    ...
    while 1:
        ...
        worker_id_q.put(worker_id)

        while(global_flags[worker_id] == 0):
            pass
...
def main(argv=None):  # pylint: disable=unused-argument
  ...
  global_flags = Array('i', range(MAX_WORKERS))
  ...
  for i in xrange(MAX_WORKERS):
        ...
        global_flags[i] = 0
        p = Process(target=handleWorker, args=(i,process_port,gradients_q,worker_id_q,global_var_vals,global_flags))
        p.daemon=True
        p.start()
